...
describe("CrossLinkStore", () => {
	...
	it("finds links by project slug", () => {
		const store = new CrossLinkStore();
		store.add({ from: "api", to: "shared", type: "depends-on", reason: "r1", symbols: [], inferred: false, confidence: 1.0 });
		store.add({ from: "web", to: "shared", type: "uses-types-from", reason: "r2", symbols: [], inferred: false, confidence: 1.0 });
		store.add({ from: "api", to: "web", type: "shares-pattern", reason: "r3", symbols: [], inferred: true, confidence: 0.7 });
	...
	it("deletes links by id", () => {
		const store = new CrossLinkStore();
		const link = store.add({ from: "a", to: "b", type: "depends-on", reason: "test", symbols: [], inferred: false, confidence: 1.0 });
	...
	it("preserves inferred flag and confidence", () => {
		const store = new CrossLinkStore();
		store.add({ from: "a", to: "b", type: "shares-pattern", reason: "shared sym", symbols: ["Foo"], inferred: true, confidence: 0.7 });
