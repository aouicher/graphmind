...
2. Run the appropriate `graphmind` CLI command via Bash, always prefixing with PATH:
   ```
   PATH="$HOME/.graphmind/bin:$HOME/.local/bin:/usr/local/bin:$PATH" graphmind <command>
   ```
