...
fn collect_symbols(node: Node, source: &str, symbols: &mut Vec<Symbol>) {
    match node.kind() {
        "class_declaration" => {
            if let Some(name_node) = node.child_by_field_name("name") {
                symbols.push(Symbol {
                    ...
                    signature: None,
                    doc: extract_javadoc(node, source),
                });
            }
        }
        ...
        "method_declaration" | "constructor_declaration" => {
            if let Some(name_node) = node.child_by_field_name("name") {
                let sig = build_java_signature(node, source);
                symbols.push(Symbol {
                    name: node_text(name_node, source),
...
fn build_java_signature(node: Node, source: &str) -> String {
...
fn collect_call_sites(node: Node, source: &str, sites: &mut Vec<CallSite>, current_fn: Option<&str>) {
    let fn_name = if node.kind() == "method_declaration" || node.kind() == "constructor_declaration" {
