...
fn extract_declarator_name(node: Node, source: &str) -> Option<String> {
    match node.kind() {
        "function_declarator" => node.child_by_field_name("declarator").map(|n| node_text(n, source)),
        "qualified_identifier" | "identifier" => Some(node_text(node, source)),
        _ => node.child_by_field_name("declarator").and_then(|d| extract_declarator_name(d, source)),
    }
}
...
fn collect_call_sites(node: Node, source: &str, sites: &mut Vec<CallSite>, current_fn: Option<&str>) {
    let fn_name = if node.kind() == "function_definition" {
        node.child_by_field_name("declarator").and_then(|d| extract_declarator_name(d, source))
    } else {
        None
    };
