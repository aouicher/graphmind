...
export function registerQueryCommand(program: Command): void {
	...
	program
		...
		.action((file: string, opts: { in?: string; depth: string }) => {
			...
			const impacted = q.impact(file, Number.parseInt(opts.depth, 10));
	...
	program
		...
		.action((symbol: string, opts: { in?: string; tests?: boolean }) => {
			...
			if (opts.tests === false) {
				callers = callers.filter((c) => !c.file.includes(".test.") && !c.file.includes(".spec."));
			}
	...
	program
		...
		.action((slug: string | undefined, opts: { limit: string }) => {
			...
			const top = q.topConnected(Number.parseInt(opts.limit, 10));
